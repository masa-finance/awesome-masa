...
# Read the funded coldkeys from funded_coldkeys.txt
funded_coldkeys = []
try:
    with open("funded_coldkeys.txt", mode='r') as funded_file:
        funded_coldkeys = [line.strip() for line in funded_file]
except FileNotFoundError:
    print("funded_coldkeys.txt not found. Proceeding with an empty list of funded coldkeys.")
...
for index, coldkey in enumerate(coldkeys_to_fund):
    try:
        ...
        # Append the coldkey to funded_coldkeys.txt
        try:
            with open("funded_coldkeys.txt", mode='a') as funded_file:
                funded_file.write(f"{coldkey}\n")
        except Exception as e:
            print(f"Failed to append {coldkey} to funded_coldkeys.txt: {e}")
