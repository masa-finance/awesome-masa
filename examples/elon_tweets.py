...
while current_date >= start_date:
    ...
    request_body = {"query": query, "count": 25}
    ...
    if response.status_code == 200:
        ...
        if response_data and 'data' in response_data and response_data['data'] is not None:
            ...
            for tweet in response_data['data']:
                zulu_time = datetime.fromtimestamp(tweet['Timestamp'], timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
                text = clean_text(tweet['Text'])
                tweets_data.append([text, zulu_time])
            logging.info(f"Fetched {num_tweets} tweets for {current_date.strftime('%Y-%m-%d')}.")
        else:
            logging.warning(f"No tweets fetched for {current_date.strftime('%Y-%m-%d')}.")
    else:
